...
export default {
  ...
  methods: {
    ...
    // 获取信息
    getInfo () {
      this.$http.get(`/product/categorybrandrelation/${this.dataForm.id}`).then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    dataFormSubmitHandle: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/product/categorybrandrelation/', this.dataForm).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
